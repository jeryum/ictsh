...
function renderMealResults(responses) {
  ...
  if (!responses || responses.length === 0) {
    container.innerHTML =
      '<p class="no-results">No meals found. Try a different search.</p>';
...
document.getElementById("letterSelect").addEventListener("change", function () {
  ...
  if (letter) {
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?f=${letter}`)
      ...
      .catch(() => {
        document.getElementById("ingredients-container").innerHTML =
          '<p class="no-results">Error fetching meals</p>';
      });
  }
});
  ...
  .addEventListener("change", function () {
    ...
    if (category) {
      fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${category}`)
        ...
        .then((data) => {
          if (data.meals) {
            fetchMealDetails(data.meals).then((detailedMeals) =>
              renderMealResults(detailedMeals),
            );
          } else {
            document.getElementById("ingredients-container").innerHTML =
...
document.getElementById("areaSelect").addEventListener("change", function () {
  ...
  if (area) {
    fetch(
      `https://www.themealdb.com/api/json/v1/1/filter.php?a=${area.charAt(0).toUpperCase() + area.slice(1)}`,
    )
      .then((response) => response.json())
      .then((data) => {
        if (data.meals) {
          ...
          document.getElementById("ingredients-container").innerHTML =
            '<p class="no-results">No meals found in this area</p>';
        }
      })
      .catch(() => {
        document.getElementById("ingredients-container").innerHTML =
...
function searchMeals() {
  ...
  if (!searchValue) {
    container.innerHTML =
      '<p class="no-results">Please enter a meal name or ID</p>';
    return;
  }
  ...
  if (!isNaN(searchValue)) {
    fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${searchValue}`)
      ...
      .catch((error) => {
        ...
        container.innerHTML =
          '<p class="no-results">Error fetching meal by ID. Please check the ID and try again.</p>';
      });
  } else {
    // Search by meal name
    fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${searchValue}`)
      ...
      .catch((error) => {
        ...
        container.innerHTML =
          '<p class="no-results">Error fetching meals. Please try again later.</p>';
      });
  }
}
  ...
  fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${mealId}`)
    ...
    .then((data) => {
      if (data.meals) {
        ...
        document.getElementById("meal-details-container").innerHTML =
          '<p class="no-results">Meal not found.</p>';
      }
    })
    .catch(() => {
      document.getElementById("meal-details-container").innerHTML =
        '<p class="no-results">Error fetching meal details.</p>';
    });
}
